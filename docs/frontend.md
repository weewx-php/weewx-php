...
The [Forecast extension](forecast.md) registers `forecast.day`,
`forecast.hourly` and `forecast.status`. These tags read local data and
use the same output profiles and unit conversions as measurements.
...
The separately installable [demo theme](themes.md#demo) uses these tags to display
...
HTTP ticks enqueue background work durably. Archiving, analytics, external
services and maintenance run in separate CLI processes. Analytics does not
hold a write lock on the live journal; cache and archive use separate databases.
Calculations resume instead of restarting on every trigger. The time budget
is derived from PHP limits, configuration and observed interruptions.

Dry spells require complete daily coverage or an unchanged, reliable rain counter
across a gap. Rain spanning several unknown days and unbridgeable resets break
a proven dry spell. The check combines measured intervals with counter evidence
for individual gaps. A month boundary outside such gaps does not discard the day.
Daily charts use the same evidence: unsupported zero totals remain unknown; the
current day ends at the last measurement and is not a completed dry day.
`weewx_rain_evidence` preserves gap evidence outside the WeeWX archive columns;
imported daily, monthly and yearly counters can also provide evidence. An expired
analytics cache does not add a blanket warning above current readings; missing
values retain their individual status.
...
Enabled packages register names in their own namespace:
...
Results are `Value`, `Series` or `Report` and support output profiles.
Readers use prepared local data. External comparisons are not registered as
station observations in analytics. See [extension registration](extensions.md)
and [climate tags](https://github.com/weewx-php/extension-climate).
